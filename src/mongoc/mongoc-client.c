...
_mongoc_client_get_srv (const char *service, bson_error_t *error)
{
   ...
   RR_ERR ("Failed to look up service \"%s\": no libresolv", service);
   ...
   if (size < 0) {
      RR_ERR (
         "Failed to look up service \"%s\": %s", service, strerror (errno));
   }

   if (ns_initparse (query_buffer, size, &ns_answer)) {
      RR_ERR (
         "Invalid SRV answer for \"%s\": \"%s\"", service, strerror (errno));
   }
